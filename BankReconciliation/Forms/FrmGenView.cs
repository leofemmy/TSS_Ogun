...
{
    public partial class FrmGenView : Form
    {
        private bool isFirst = false;

        private string monthName;

        public static FrmGenView publicStreetGroup;
        ...
        public FrmGenView()
        {
            ...
            //cboPeriods.KeyPress += cboPeriods_KeyPress;

            //cboYears.KeyPress += cboYears_KeyPress;
        }
        ...
        void GetPeriodCollection(string param)
        {
            string query = String.Format("select sum(Amount) as Amount from ViewCollectionAmountBankCode where  (CONVERT(VARCHAR, CONVERT(DATE, PaymentDate), 102) BETWEEN'{0}' and '{1}') and IsRecordExit=1 and IsPayDirect=1 ", string.Format("{0:yyyy.MM.dd}", dtpStart.Value), string.Format("{0:yyyy.MM.dd}", dtpEnd.Value));

            //string qurys = String.Format("SELECT SUM(Amount) AS Amount FROM dbo.ViewCollectionAmountBankCode WHERE permonth ='{0}' AND peryear ='{1}'", cboPeriods.SelectedValue, cboYears.SelectedValue);
            ...
            if (dts != null && dts.Rows.Count > 0)
            {
                dbCollect = Convert.ToDouble(dts.Rows[0]["Amount"]);
            }
        ...
        private XtraReport CreateReport()
        {
            ...
            XRPivotGridField fieldCategoryName = new XRPivotGridField("BankName", PivotArea.RowArea) { AreaIndex = 0, Width = 150 };

            fieldCategoryName.CellFormat.FormatType = DevExpress.Utils.FormatType.Custom;
            fieldCategoryName.CellFormat.FormatString = "({0})";

            XRPivotGridField fieldCountry = new XRPivotGridField("TransDescription", PivotArea.ColumnArea) { AreaIndex = 0, Width = 120 };


            XRPivotGridField fieldQuantity = new XRPivotGridField("Amount", PivotArea.DataArea) { AreaIndex = 0, Width = 80 };
