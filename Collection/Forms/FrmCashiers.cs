...
namespace Collection.Forms
{
    public partial class FrmCashiers : Form
    {
        ...
        public FrmCashiers()
        {
            ...
            chkPeriod.CheckedChanged += chkPeriod_CheckedChanged;

            //NavBars.ToolStripEnableDisable(toolStrip, null, false);
        }

        void chkPeriod_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private void searchRecord()
        {
            ...
            if (chkPeriod.Checked)
            {
                querys = String.Format("SELECT PaymentRefNumber, PaymentDate, DepositSlipNumber AS [Slip No], Amount, BankName, BranchName FROM tblCollectionReport where PaymentDate between '{0:dd-MM-yyyy}' and '{1:dd-MM-yyyy}' ", dtpfrom.Value.Date, dtpTo.Value.Date);
        ...
        public void setDBComboBoxPerson()
        {
            ...
            using (var ds = new System.Data.DataSet())
            {
                ...
                string query = "select TaxPayerReferenceNumber as [Payer Name],UPPER(Surname) + ' ' + UPPER(Othernames)  as fullname from tblTaxPayer";
