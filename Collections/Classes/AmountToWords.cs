...
public class AmountToWords
{
    ...
    public string convertToWords(string amount, string prefix, string surfix)
    {
        ...
        if (numbDec == 0)
        {
            englishTranslation = string.Format("{0} {1}", NumberToWords(numb), prefix);
        }
        ...
        if (numbDec != 0)
        {
            englishTranslation = String.Format("{0} {1} {2} {3} Only.", NumberToWords(numb), prefix, NumberToWords(numbDec), surfix);
        }
