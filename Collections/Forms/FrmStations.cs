...
namespace Collection.Forms
{
    public partial class FrmStations : Form
    {
        ...
        public static FrmStations publicInstance;
        ...
        private void setReload()
        {
            ...
            using (var ds = new System.Data.DataSet())
            {
                ...
                using (SqlDataAdapter ada = new SqlDataAdapter("select * from Receipt.tblStation", Logic.ConnectionString))
                {
                    ada.Fill(ds, "table");
                }
                //connect.connect.Close();
        ...
        private bool FillField(int fieldid)
        {
            ...
            DataTable dts = (new Logic()).getSqlStatement((String.Format("select * from Receipt.tblStation where StationID ='{0}'", fieldid))).Tables[0];

            if (dts != null)
            {
                bResponse = true;
                txtStation.Text = dts.Rows[0]["StationCode"].ToString();
                txtstationName.Text = dts.Rows[0]["StationName"].ToString();
